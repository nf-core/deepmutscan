...
run_mutscan_fitness_estimation <- function(counts_path,
  ...
  plotPairs(se, selAssay = "counts", addIdentityLine = TRUE)
  dev.off()
  
  # edgeR volcano plot
  pdf('mutscan_edgeR_volcano.pdf', height = 9, width = 14)
  plotVolcano(logFC.edgeR, pointSize = "large")
  dev.off()
  
  # limma volcano plot
  pdf('mutscan_limma_volcano.pdf', height = 9, width = 14)
  plotVolcano(logFC.limma, pointSize = "large")
  dev.off()
